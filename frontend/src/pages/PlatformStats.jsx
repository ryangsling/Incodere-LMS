...
export default function PlatformStats() {
  ...
  if (loading) return <p className="text-[#888888]">Loading stats...</p>
  if (error) return <p className="text-red-600">{error}</p>
  if (!stats) return <p className="text-[#888888]">No stats available.</p>

  const cards = [
    { label: 'Organisations', value: stats.total_organisations, color: 'bg-blue-100 text-blue-600' },
    { label: 'Active Learners', value: stats.total_learners, color: 'bg-green-100 text-[#437a22]' },
    { label: 'Total Courses', value: stats.total_courses, color: 'bg-purple-100 text-purple-600' },
    { label: 'Published Courses', value: stats.published_courses, color: 'bg-teal-100 text-[#01696f]' },
    { label: 'Certificates Issued', value: stats.total_certificates, color: 'bg-orange-100 text-orange-600' },
  ]

  return (
    <div>
      <h2 className="text-xl font-bold text-[#032147] mb-6">Platform Statistics</h2>
