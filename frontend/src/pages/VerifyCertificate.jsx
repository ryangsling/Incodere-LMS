        ...
        {data?.valid && (
          <Card>
            <CardBody className="text-center">
              ...
              <p className="text-sm text-ink/60 mb-4">has successfully completed</p>
              <p className="text-base font-semibold text-accent mb-4">{data.course_title}</p>
              <p className="text-xs text-ink/60">Issued on {new Date(data.issued_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
              <p className="text-xs text-ink/60 mt-1">Certificate ID: {data.certificate_id}</p>
              {user && (
                <button
                  type="button"
