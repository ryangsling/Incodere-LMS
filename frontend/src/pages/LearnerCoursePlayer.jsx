...
export default function LearnerCoursePlayer() {
  ...
  if (loading) return <p className="text-muted p-6">Loading course...</p>
  if (error) return <p className="text-red-600 p-6">{error}</p>
  if (!course) return <p className="text-muted p-6">Course not found</p>
    ...
    <div className="min-h-screen bg-canvas flex flex-col">
      <header className="bg-primary-600 text-white px-4 sm:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <button onClick={logout} className="text-xs sm:text-sm bg-white text-primary-600 px-2 py-0.5 rounded">Sign Out</button>
        </div>
      </header>
        ...
        <aside className={`${sidebarOpen ? 'block' : 'hidden'} sm:block w-full sm:w-64 bg-white border-r border-gray-200 overflow-y-auto p-4 absolute sm:relative z-10 sm:z-auto min-h-full shadow-lg sm:shadow-none`}>
          ...
          {course.sections?.map(section => (
            <div key={section.id} className="mb-4">
              <h3 className="text-xs font-bold text-navy-700 uppercase mb-2">{section.title}</h3>
        ...
        <main className="flex-1 p-4 sm:p-6 overflow-y-auto">
          {activeLesson ? (
            <div className="max-w-3xl mx-auto">
              <h2 className="text-lg sm:text-xl font-bold text-navy-700 mb-4">{activeLesson.title}</h2>
            ...
            <p className="text-muted text-center mt-20">No lessons in this course.</p>
          )}
          ...
          {totalLessons > 0 && pct === 100 && (
            <div className="max-w-3xl mx-auto mt-6 text-center">
              <button
                ...
                className="bg-accent-600 text-white px-6 py-3 rounded text-sm hover:bg-accent-700 font-medium"
              >
                Get Certificate
              </button>
            </div>
