...
export async function generateCertificate(req, res) {
  ...
  const verifyUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/verify/`

  const { error: emailErr } = await resend.emails.send({
    from: 'ILMS <onboarding@resend.dev>',
    to: user.email,
    subject: `Certificate of Completion - ${course.title}`,
    html: `<p>Congratulations ${user.first_name},</p><p>You have completed <strong>${course.title}</strong>.</p><p>Your certificate is attached.</p><p>Verify at: <a href="${verifyUrl}">ILMS verify</a></p>`,
  ...
  res.status(201).json({ success: true, data: { id: certRow.id, course_id, learner_id: userId, issued_at: new Date().toISOString() } })
}
...
export async function downloadCertificate(req, res) {
  ...
  // Authorise: owner learner, company admin in the same org, or super admin.
  let allowed = req.user.role === 'super_admin'
  if (!allowed) {
    if (req.user.id === cert.learner_id) {
      allowed = true
      ...
      const { data: learner } = await supabase
        ...
        .maybeSingle()
      allowed = learner?.organisation_id === req.user.organisation_id
    }
  }
  if (!allowed) return res.status(403).json({ success: false, error: 'Not allowed to access this certificate' })
  ...
  const { data: user } = await supabase
    ...
    .eq('id', cert.learner_id)
    .single()
  if (!user) return res.status(500).json({ success: false, error: 'User not found' })
  ...
  const { data: course } = await supabase
    ...
    .single()
  if (!course) return res.status(500).json({ success: false, error: 'Course not found' })
...
async function buildPdf(firstName, lastName, courseTitle) {
  ...
  const issuedWidth = font.widthOfTextAtSize('Issued by incodet - verify at incodet.com/ilms/verify', 9)
  page.drawText('Issued by incodet - verify at incodet.com/ilms/verify', { x: (width - issuedWidth) / 2, y: 30, size: 9, font, color: white })
...
export async function listMyCertificates(req, res) {
  ...
  res.json({ success: true, data: { rows: result, total: result.length } })
}
...
export async function listCertificates(req, res) {
  ...
  let query = supabase
    ...
    .select(`
      *,
      learner:users(first_name, last_name, email, organisation_id),
  ...
  if (req.user.role === 'company_admin') {
    ...
    if (ids.length === 0) return res.json({ success: true, data: { rows: [], total: 0, page, pageSize } })
  ...
  res.json({ success: true, data: { rows: result, total: result.length, page, pageSize } })
}
